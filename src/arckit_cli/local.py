...
def _get_config_path() -> Path:
    """Return the path of the config file load/save use (project-level when
    present, otherwise the global user config)."""
    return resolve_config_path() or get_global_config_path()


def _load_config() -> dict:
    """Load the resolved ArcKit config dict (project-level when present,
...
def setup(
    ...
    # Parse with fallback so bad input never crashes the wizard
    try:
        parsed_max = int(max_tokens_str)
    except (TypeError, ValueError):
        console.print("[yellow]Invalid max tokens — using default 128000.[/yellow]")
        parsed_max = 128000
    try:
        parsed_temp = float(temp_str)
    except (TypeError, ValueError):
        console.print("[yellow]Invalid temperature — using default 0.0.[/yellow]")
        parsed_temp = 0.0
    ...
    console.print("\n[dim]Ready. Use:[/dim]")
    console.print("  arckit init my-project --ai opencode")
    console.print("  cd my-project && arckit build --recipe strategy")
...
def status():
    ...
    table.add_row("Context Window", str(context_window))
...
@local_app.command()
def test(
    model: str | None = typer.Option(None, "--model", help="Override model from config"),
    url: str | None = typer.Option(None, "--url", help="Override base_url from config"),
):
