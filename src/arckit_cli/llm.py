...
from arckit_cli.config import load_config as _resolve_config
from arckit_cli.recipe import Target


def _load_config():
    """Load the resolved ArcKit config (project-level when present,
    otherwise the global user config; empty dict when no file exists)."""
...
def resolve_config(
    ...
    Resolution precedence:
        ...
        2. ArcKit config file (project-level .arckit/config.yaml or
           arckit.yaml, else the global user config — e.g.
    ...
    try:
        ...
        temperature = 0.0
    try:
        context_window = int(llm_cfg.get("context_window", 128000))
    except (TypeError, ValueError):
...
# Reserve tokens for chat-template overhead plus a safety margin, and a
# floor so the model can always produce a minimal completion.
_CONTEXT_RESERVE_TOKENS = 1024
_MIN_COMPLETION_TOKENS = 256


def _estimate_prompt_tokens(messages: list[dict], tools: list[dict]) -> int:
    """Conservative estimate of the prompt's token count.

    Sums the character length of all message content plus the serialised
    tool schemas, then applies a ~3 chars/token factor (a conservative
    upper bound for mixed prose/code). Over-estimating the prompt keeps
    ``prompt + completion`` safely under the model context window.
    """
...
def _fit_max_tokens(
    messages: list[dict], tools: list[dict], config: "LLMConfig"
) -> int:
    """Cap ``max_tokens`` so the request fits inside the context window.

    OpenAI-compatible endpoints reject requests where
    ``prompt_tokens + max_tokens`` exceeds the model context window.
    Because ``max_tokens`` can default to the full window, subtract the
    estimated prompt size (plus a reserve) from the completion budget.
    """
    prompt_tokens = _estimate_prompt_tokens(messages, tools)
    budget = config.context_window - prompt_tokens - _CONTEXT_RESERVE_TOKENS
    return max(_MIN_COMPLETION_TOKENS, min(config.max_tokens, budget))
...
async def call_llm(
    ...
    body: dict = {
        ...
        "messages": messages,
        "tools": tools,
        "max_tokens": _fit_max_tokens(messages, tools, config),
    ...
    try:
        ...
        for _iteration in range(max_iterations):
            ...
            for tc in tool_calls:
                ...
                # Track written paths
                if tc["function"]["name"] == "Write" and tool_result.startswith("Written to "):
