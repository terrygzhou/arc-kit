...
def init(
    ...
    if not ai_assistant:
        ...
        console.print("[dim]  /plugin marketplace add terrygzhou/arc-kit[/dim]")
    ...
    if ai_assistant == "claude":
        ...
        console.print("  [cyan]/plugin marketplace add terrygzhou/arc-kit[/cyan]")
...
# Config helpers
# ---------------------------------------------------------------------------
from arckit_cli.config import ConfigError, load_config, save_config
...
def _save_config(cfg):
    """Write config dict to the resolved YAML file (project-level when present,
    otherwise the global user config; creates parent dir if needed)."""
...
_CONFIG_KEYS = [
    ...
    "llm.max_tokens", "llm.temperature", "llm.context_window",
...
def build(
    ...
    for wave in waves:
        ...
        for t in wave.targets:
            ...
            _ts = state.targets.get(t.id)
            if resume and _ts is not None and _ts.status == "complete":
                skip_count += 1
                continue
            if refresh_set and t.id not in refresh_set:
                ...
                if _ts is not None and _ts.status == "complete":
                    skip_count += 1
                    continue
