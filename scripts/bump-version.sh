...
# NOTE: org-agnostic — preserves the existing GitHub org (works for forks too,
# e.g. terrygzhou/arc-kit) instead of hardcoding tractorjuice.
sed -i -E "s/\\[v[0-9]+\\.[0-9]+\\.[0-9]+\\]\\(https:\\/\\/github\\.com\\/([A-Za-z0-9_.-]+)\\/arc-kit\\/releases\\/tag\\/v[0-9]+\\.[0-9]+\\.[0-9]+\\)/\\[v${NEW_VERSION}\\](https:\\/\\/github.com\\/\\1\\/arc-kit\\/releases\\/tag\\/v${NEW_VERSION}\\)/g" README.md
...
# ── 18. Regenerate derived trees (standalone mirror + converter configs) ──────
#
# Source VERSION files are authoritative, but two trees are *generated* from
# them and would silently drift behind a source-only bump:
#   - plugins/arckit-claude/plugins/**/VERSION + .claude-plugin/plugin.json
#       (mirrored by scripts/sync-claude-plugin-layout.py)
#   - extensions/arckit-vibe/vibe-config.toml (+ extensions/arckit-kimi/*)
#       (generated by scripts/converter.py from each extension's VERSION file)
# Refreshing them here keeps the release tree internally consistent and stops
# "standalone VERSION drifted behind root VERSION" release-test failures.

echo ""
echo "── Regenerating derived trees ──"
echo ""
python3 scripts/sync-claude-plugin-layout.py
python3 scripts/converter.py
echo ""
echo "  ✓ derived trees refreshed (standalone mirror + vibe/kimi configs)"
echo ""
