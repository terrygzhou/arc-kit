...
*To compute exact deltas: run `gh api repos/terrygzhou/arc-kit/stargazers -H "Accept: application/vnd.github.star+json" --paginate` against the live CLI — unavailable in this MCP session.*
...
```bash
gh api repos/terrygzhou/arc-kit/forks --paginate | jq '.[] | {full_name, stargazers_count}'
```
