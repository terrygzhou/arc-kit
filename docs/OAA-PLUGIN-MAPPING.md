...
| **Core foundation** | `plugins/arckit-claude/commands/` | Foundation commands the OAA recipe depends on: `principles` (PRIN), `requirements` (REQ), `stakeholders` (STKE), plus `backlog` (BKLG), `strategy` (STRAT), `roadmap` (ROAD), `adr` (ADR), `health`, `traceability` (TRAC), `risk` (RISK), and `bmm` (BMM) — the BMM 1.3 Business Motivation Model whose Strategic Themes feed `OASTR` (agile strategy) and the ADM `discovery` motivation views |
