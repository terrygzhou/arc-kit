...
- **`product-architecture.json`** — Product architecture schema (shared with `/arckit:agile-strategy`)

- **`vision.yaml`** — Architecture vision from O-AA Lite or ADM Preliminary (shared with `/arckit:oaa-adm-lite`, `/arckit:adm-preliminary`)
...
2. Fill the `## PlantUML ArchiMate View` block in the template, stereotyped into the **Application** layer focus.
...
This is additive — the existing Mermaid C4 component diagram are retained, not replaced.
