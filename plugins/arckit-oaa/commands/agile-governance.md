...
- **`change-request.yaml`** — Architecture change request schema (shared with `/arckit:architecture-change`)

- **`compliance-evidence.json`** — Sprint compliance evidence (shared with `/arckit:agile-security`)

- **`vision.yaml`** — Governance constraints from architecture vision (shared with `/arckit:oaa-adm-lite`)
...
4. **Shared Schemas**: The `change-request.yaml` and `compliance-evidence.json` schemas are shared across O-AA commands and with traditional TOGAF commands (like `/arckit:architecture-change`). This ensures consistency regardless of governance approach.
