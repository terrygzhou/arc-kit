...
- **DATA** (Data Architecture) — Extract: Data platform requirements, storage needs, data processing requirements, data security classifications
  - If missing: STOP and ask user to run `/arckit-togaf-adm:data-architecture` first. Technology architecture must support data architecture requirements.
...
2. Perform gap analysis against current state: `/arckit-togaf-adm:gap-analysis`
3. Plan migration to target technology state: `/arckit-togaf-adm:transition-architecture`
...
7. **Integration with Other Commands**:
   ...
   - **Output**: Feeds `/arckit-togaf-adm:gap-analysis` (current vs. target gaps), `/arckit-togaf-adm:transition-architecture` (migration planning)
