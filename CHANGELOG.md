...
- **Business Motivation Model adoption: new `/arckit:bmm` command, `BMM` doc-type, and BMM-projection diagram layer (OpenSpec `bmm-adoption`).** `/arckit:bmm` (`effort: max`) generates the project's **Business Motivation Model (BMM 1.3)** — the OMG metamodel behind BIZBOK *Strategy & Stakeholder Alignment* and the ArchiMate Motivation/Strategy tiers — as a single canonical per-project artefact, `projects/{p}/ARC-{P}-BMM-v1.0.md` (single-instance, root-level; `BMM` registered in `DOC_TYPES` category *Planning* only — deliberately absent from `MULTI_INSTANCE_TYPES`/`SUBDIR_MAP` so regeneration bumps the version instead of sequencing). The model is rendered from `templates/bmm-template.md` (pipeline sections: stakeholders & outcomes + stakeholder–outcome matrix; Outcomes → Goals → Objectives → Measures ladder; typed drivers; explicit assumptions; Case For / Case Against; impact factors; Strategic Themes → Capabilities → Courses of Action → Resources; traceability + external references; Mermaid mindmap companion block; canonical 14-field Document Control + generation footer). The model is then **projected** to two sequenced base `ARCH` ArchiMate views under `projects/{p}/diagrams/` (Motivation; Strategy/Capability) with an optional companion menu (goal/objective ladder, case & assumptions overlay, alignment/traceability view, Mermaid mindmap), each rendered offline to self-contained `.svg` with the pinned `plantuml-1.2026.8.jar -tsvg` delivery clause. `references/bmm-reference.md` (shipped in all 17 plugin trees) carries the BMM 1.3 element/relationship catalogue, BMM ↔ ArchiMate ↔ ArcKit mapping (STKE/SOBC/STRAT/ROAD/BPCM/ARCH), BIZBOK S&SA + UK Green-Book alignment notes, and gate criteria **G1–G5** with the 3-iteration remediation loop. `skills/plantuml-syntax/references/archimate.md` gains an append-only `## BMM Projection` section — stereotyped encodings for the 4.0-only elements (`Objective`, `Measure`, `Strategic Theme`) plus the `Rel_Influence`/`Rel_Realization`/`Rel_Composition`/`Rel_Flow` relationship vocabulary and legend rules — with rendered spike fixtures (`tests/fixtures/archimate/bmm/*.puml` + committed self-contained SVGs) and pre-change sections byte-preserved against a SHA-256 fixture. The BMM section ships in every tracked `quality-checklist.md` copy (31+ via `sync-shared-assets.py`); `strategy.md` gains the Strategic-Theme consumption clause (STRAT consumes BMM Strategic Themes when present) and `sobc.md` the 5-case ↔ BMM Case-section mapping (UK-regime Green-Book note); `docs/guides/bmm.md` (+ byte-identical plugin-tree copy, site links, `llms.txt` entry, OAA-PLUGIN-MAPPING pointer; guide category *Planning*, beta). Guarded by `tests/plugin/test_bmm_conformance.py` (37 checks: dual registration, command contract, template, reference, append-only diagram layer, lockstep) plus the intake ask-always guards.

- **BPCM capability map rendered top-down, complexity-adaptively (agent rule).** The BPCM capability-map ArchiMate view now chooses its *diagram set* from model complexity, always built top-down (`LAYOUT_TOP_DOWN()`; a parent reads above its children). **Simple** models (total elements `E <= 12` across all present levels) flatten into a **single** diagram of every level; **complex** models (`E > 12`, or a single level denser than ~12) are **abstracted level-by-level** — an **abstract** L1-only overview (each domain with a child-count roll-up) plus **refinement** diagrams, one per L1 domain (its L2 sub-hierarchy + L3 when defined), each gated to <= 12 elements/layer and split at a capability-domain/sub-domain boundary if still dense. Lands in the `business-capability-map.md` directive, the capability-map template (flattened + `{capmap_overview*}` + `{capmap_refinement*}` placeholders), and the ArchiMate reference (level-by-level idiom). OpenSpec `bpcm-capmap-level-refinement`; guarded by `tests/plugin/test_archimate_demanded.py` (`test_bpcm_capmap_top_down_and_regimes_in_command`, `test_bpcm_capmap_regime_placeholders_in_template`).
- **BPCM capability map rendered as a PlantUML-ArchiMate hierarchical view (additive).** The business-capability-map (BPCM) capability *hierarchy* (L1 domains → L2 sub-capabilities → optional L3 detailed capabilities) is now representable as an ArchiMate Strategy-tier view, in addition to the existing Mermaid mindmap and the capability→target realization view. `plugins/arckit-togaf-adm/templates/capability-map-template.md` gains a dedicated `## Capability Map (ArchiMate View)` block: nested `Strategy_Capability` elements joined whole→part by `Rel_Composition(parent, child, "contains")` (`Rel_Aggregation` where a sub-capability is standalone), L3 optional, `LAYOUT_TOP_DOWN()`, the ≤ 12-element/layer split-never-drop gate (split at a capability-domain boundary into an additional sequenced `ARCH` document), and the pinned self-contained `.svg` delivery clause (`plantuml-1.2026.8.jar -tsvg`, offline). The `business-capability-map.md` command gains the matching capability-map-view directive (whole→part edge rule, Level-3-optional, ≤ 12 split gate, SVG render clause); `skills/plantuml-syntax/references/archimate.md` gains a `## Capability-Map ArchiMate Nesting` note. All edits are append-only and byte-preserve the Mermaid blocks. OpenSpec `bpcm-capmap-archimate`; guarded by `tests/plugin/test_archimate_demanded.py` (`test_bpcm_capmap_archimate_view_in_template`, `test_bpcm_capmap_directive_in_command`).

- **ADM/OAA ArchiMate views render to self-contained SVGs (delivery clause wired into 17 base + 4 companion directives).** PlantUML does not render in GitHub markdown, so the ArchiMate base and companion views added by `archimate-coverage-expansion` previously shipped as inline ` ```plantuml ` source with no rendered output. This wires the shipped `archimate-svg-delivery` policy into every ArchiMate-carrying ADM/OAA directive: all **17 base-view** and **4 companion-view** `arckit-togaf-adm`/`arckit-oaa` commands gain a `## Render the ArchiMate view(s) to self-contained SVG(s)` clause (offline render with the pinned build `java -jar plantuml-1.2026.8.jar -tsvg`; inline source kept as source of truth; the `.svg` is the only new rendered file — no new architecture document; self-containment check: no non-W3C `http(s)` URLs, local `#anchor` `xlink:href` only, fully offline-openable), and each affected template "View this diagram" section gains an "Artefact delivery: self-contained `.svg`" note (base + the 4 companion sections). All edits are append-only / additive and byte-preserve Mermaid; no new doc-type, command, or registry entry; `extensions/*` regenerated. Guarded by `tests/plugin/test_archimate_demanded.py` (SVG-render clause in all 17 base + 4 companion commands; `.svg` delivery note in all 17 base templates, ≥ 2 in the 4 companion templates).
- **PlantUML-ArchiMate coverage expansion: 17-artefact demanded base set + 4 Batch-1 companion views (append-only).** The 7-artefact demanded base set (OpenSpec `archimate-demanded-artefacts`) is extended to the full **17-artefact** demanded set: the ten previously non-demanded ADM/OAA artefacts (`adm-preliminary` → Strategy scope/drivers, `architecture-board` → Strategy/governance, `architecture-change` → Implementation increments, `architecture-repository` → Structure/catalog, `data-architecture` → Application/Technology data objects, `discovery` → Motivation drivers/goals, `gap-analysis` → Capability target vs current; `agile-governance` → Strategy/governance capability, `agile-security` → Technology/Application security, `agile-strategy` → Strategy capabilities/value stream) each gain the same additive `## PlantUML ArchiMate View` template section (pinned `!include <archimate/Archimate>`, PlantUML 1.2026.8; `{diagram_title}`/`{plantuml_*}` tokens) plus the matching `## PlantUML ArchiMate View (additive)` command directive. **Companion views (Batch 1):** `transition-architecture` + `gap-analysis` gain a separate **Implementation & Migration** companion `### PlantUML ArchiMate Companion View` block, and `technology-architecture` + `oaa-adm-lite` gain a separate **Physical** companion block — each a second pinned `@startuml` block emitted as a separate sequenced `ARCH` document (never merged into the base view), gated by the ≤ 12 elements/layer, realization concrete→abstract, split-never-drop invariants. All edits are **append-only and byte-preserve every existing Mermaid block** (SHA-256 fixture extended to the in-scope ADM mermaid-carrying templates); no new doc-type, command, or registry entry. A fixed **8-type optional menu** (3 overlays: Motivation / Strategy-Capability / Data-only; 5 companion single-layer views: Behavior / Structure / Physical / Implementation & Migration / Stakeholder-Requirement) with a per-artefact mapping is captured spec-only in `openspec/changes/archimate-coverage-expansion/specs/artifact-generation/spec.md` (no code). Guarded by `tests/plugin/test_archimate_demanded.py` (all-17 base-view blocks, Batch-1 companion blocks, base-view directives, mermaid-preserved vs the extended fixture) alongside `tests/plugin/test_archimate_conformance.py` (unchanged, still green).

- **Demanded PlantUML-ArchiMate views on 7 ADM/OAA artefacts (Option A, append-only).** Five `arckit-togaf-adm` artefacts (application-inventory → Application, application-rationalization → Application+Capability, business-capability-map → Business/Capability, technology-architecture → Technology, transition-architecture → Capability/incremental target) and two `arckit-oaa` artefacts (oaa-adm-lite → Technology+Application, product-architecture → Application) now carry an additive `## PlantUML ArchiMate View` section in their source template plus a matching `## PlantUML ArchiMate View (additive)` directive in their source command. The block is pinned to the ArchiMate stdlib include `!include <archimate/Archimate>` (PlantUML 1.2026.8) with `{diagram_title}`, `{plantuml_elements}`, `{plantuml_relationships}` (realization concrete→abstract), and `{plantuml_layout}` + `LAYOUT_TOP_DOWN()` tokens, and reuses `skills/plantuml-syntax/references/archimate.md` — no new doc-type, command, or registry change. Mermaid blocks are byte-preserved (SHA-256 snapshotted per template; OAA `.mmd`/C4 Mermaid deliverables stay additive, not replaced). Guarded by `tests/plugin/test_archimate_demanded.py` (block-in-template, directive-in-command, mermaid-preserved, fixture well-formedness) with fixture `tests/plugin/fixtures/archimate-demanded/prechange_mermaid_blocks.json`.

- **ArchiMate layer views via `/arckit:archimate` (new `ARCH` doc type, PlantUML notation).** New `commands/archimate.md` generates single-layer ArchiMate views (Business / Application / Technology, with an optional Motivation or Strategy overlay) in PlantUML ArchiMate notation using the pinned native stdlib include `!include <archimate/Archimate>` — pinned 2026-09-13 against PlantUML 1.2026.8, with the full pinned API (61 element macros, typed `Rel_*` relationship macros with direction suffixes, layout helpers, theme colour tokens) and render-truth fixture recorded in `skills/plantuml-syntax/references/archimate.md` (`tests/fixtures/archimate/three-layer.puml` + committed SVG render). `ARCH` (ArchiMate View) is registered as a multi-instance type in `config/doc-types.mjs` (`DOC_TYPES` + `MULTI_INSTANCE_TYPES` + `SUBDIR_MAP['diagrams']`), the `commands/pages.md` allow-list, and both `generate-document-id.sh` copies, so views sequence as `ARC-{P}-ARCH-{NNN}-v1.0.md` under `projects/{p}/diagrams/` (never D2 sidecars — `docs/DIAGRAMS.md` documents the mapping: generated ArchiMate views are PlantUML artefacts; the D2 ArchiMate idiom stays hand-authored sidecars only). Each view passes a 9-criterion ArchiMate quality gate (single-layer stereotyping, realization edges concrete → abstract, ≤ 12 elements/layer, labelled edges, notation purity, pinned include, layout consistency, custom-notation legend, inventory traceability) with a 3-iteration remediation loop, renders from `templates/archimate-template.md`, and wires the shared intake interview. The `ARCH` section ships in every tracked `quality-checklist.md` copy (incl. the customised OAA copy), the `plantuml-syntax` skill gains the ArchiMate mapping row + `**/ARC-*-ARCH-*.md` path glob, and 12 stale nested `intake-instructions.md` mirrors were re-synced in passing. Guarded by `tests/plugin/test_archimate_conformance.py` (registry, bash parity, template pin, command frontmatter + intake wiring, checklist lockstep, `docs/DIAGRAMS.md` mapping).

- **Diagram sidecar layer: D2 corpus + archify showcase, CI-checked.** Six D2 sidecars under `projects/000-global/diagrams/` (one per major artefact: BPCM-v2.0, TECH-v1.0, TRANS-v1.0, DATA-v1.0, APP-v2.0, GAPA-v2.0) mirror the mermaid diagrams in the corresponding `ARC-*.md` artefacts and ship local `svg/` renders (`projects/` is gitignored — the sidecars are local-only; what is tracked is the corpus convention, the CI checks, and this entry); ArchiMate semantics are encoded as a layer-container + "realizes"-edge idiom (reference: `arc-000-tech-v1.0.d2`). archify showcase specs (`.workflow.json` → `.workflow.html`) are optional per artefact — the TRANS showcase is delivered at **standard** quality, with its one unresolvable edge crossing documented rather than claimed at professional. `scripts/diagrams.sh check|render` validates/renders the whole layer (old-d2-build detection with a compile-only fallback; missing toolchains SKIP, never fail), and is wired into `scripts/ci-local.sh` as a new "diagram sidecar checks" step. Conventions, naming, and quality bar are documented in `docs/DIAGRAMS.md`.
...
- **Shipped ADM templates now render checklist-conforming artefacts (14-field Document Control, canonical revision-history columns, valid mermaid).** The 2026-09-02 conformance review of `test-adm-australian-post` found generated ADM artefacts drifting from the quality checklist: 8 of 11 ADM templates carried 7-field Document Control tables (checklist common check #1 requires all 14 fields, so template-driven generation failed it by construction), the 32-copy checklist wording of check #6 named revision-history columns no template or artefact uses, and four templates (`capability-map`, `gap-analysis`, `application-inventory`, `agent-maturity`, 8 copies incl. mirrors) shipped `quadrantChart` blocks that fail to parse (`x-axis__`/`y-axis__` typos, comma-form plot points). All 10 ADM templates with a Document Control section — in both the `arckit` and `arckit-togaf-adm` trees — now list the full 14-field table in checklist order (`architecture-repository` also renames `Standard ID` → `Document ID` and drops a duplicate Owner row; `architecture-change` gains `Version` + `Review Cycle`), the four template mermaid blocks are corrected to valid syntax, and check #6 in all 31 tracked `quality-checklist.md` copies plus `scripts/autoresearch/program.md` now names the canonical columns `Version, Date, Author, Description, Reviewer, Approver` — the majority position shared by every template and the generated artefacts. A new TDD guard, `tests/plugin/test_adm_doc_control_conformance.py`, pins the 14-field table, the revision-history header, the mermaid axis/point syntax, and the checklist wording. The discovery (DISC) template is deliberately out of scope (no Document Control section, placeholder rendering path) and the OAA templates are a flagged follow-up.
...
- **Claude Code Plugin Migration**: Claude Code distribution migrated from CLI to standalone plugin (`arckit-claude/`). Claude Code users should install via `/plugin marketplace add terrygzhou/arc-kit` instead of `arckit init --ai claude`
...
- [v1.3.0](https://github.com/terrygzhou/arc-kit/releases/tag/v1.3.0) - External Document Support & Dynamic Version Placeholders
- [v1.2.0](https://github.com/terrygzhou/arc-kit/releases/tag/v1.2.0) - Autonomous Agent System
- [v1.0.0](https://github.com/terrygzhou/arc-kit/releases/tag/v1.0.0) - Production Release - Enterprise Architecture Governance Toolkit
- [v0.3.1](https://github.com/terrygzhou/arc-kit/releases/tag/v0.3.1) - Data Modeling with ERD, GDPR Compliance, and Data Governance
- [v0.3.0](https://github.com/terrygzhou/arc-kit/releases/tag/v0.3.0) - Green Book & Orange Book Edition (SOBC + Risk Management)
- [v0.2.2](https://github.com/terrygzhou/arc-kit/releases/tag/v0.2.2) - OpenAI Codex CLI Support & Enhanced Stakeholder Analysis
- [v0.2.1](https://github.com/terrygzhou/arc-kit/releases/tag/v0.2.1) - Stakeholder Analysis & Conflict Resolution
- [v0.2.0](https://github.com/terrygzhou/arc-kit/releases/tag/v0.2.0) - UK Government Compliance Edition
- [v0.1.0](https://github.com/terrygzhou/arc-kit/releases/tag/v0.1.0) - Initial Release
