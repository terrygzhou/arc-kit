...
- **Standard-aligned interview coverage (TOGAF + OAA).** Each OAA interview SHALL derive questions from a canonical discovery-dimension checklist — D1 business vision & strategy, D2 business capabilities, D3 stakeholders & goals, D4 constraints & drivers, D5 current-state (As-Is) architecture, D6 technology landscape, D7 data & classification, D8 pain points / gaps / risks, D9 OAA outcome dimensions, D10 OAA axioms — in addition to the template's own sections. Prefilled dimensions are silent; an unresolvable dimension is asked (grouped, skippable). This makes the interview standards-aligned even when a template section is thin, and adds no diagram/output mandate.
