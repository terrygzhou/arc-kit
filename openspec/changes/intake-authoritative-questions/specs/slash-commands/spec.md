# slash-commands Delta — intake-authoritative-questions

## MODIFIED Requirements

### Requirement: Template-Driven Intake Interview Before Artefact Generation
Before rendering its artefact, every artefact-producing command (core or bundled
overlay) SHALL run a template-driven intake interview: (1) resolve the *effective*
template (`.arckit/templates-custom/{name}-template.{ext}` first, then the
shipped default), (2) derive the inputs the artefact needs from that template's
sections, its Document Control fields not resolvable from `user_config`, and the
command's MANDATORY prerequisite inputs, (3) prefill each input from, in order,
existing artefacts already in `projects/`, previously saved intake answers, then
`user_config`, and (4) put **every** derived input to the user for their input,
one question at a time — prefilled where available so the user can confirm or
override the value — with each question **optional** and skippable. The interview
is **ask-always, answer-optional**: the *asking* of every question is mandatory,
while each *answer* is optional (a skipped question renders as a `TBD` marker).
In addition to the template-derived inputs, **authoritative question lists** are
also asked-always: if the effective template carries an `## Intake Interview
Questions` section, every question in that block is authoritative and MUST be put
to the user (prefilled where available, skippable); for `oaa` overlay commands
the discovery-dimension checklist (`intake-discovery-dimensions.md`, D1–D10) is
the canonical coverage floor and its dimensions are likewise asked-always.
Answers SHALL be persisted to `projects/{NNN}-{slug}/.arckit/intake/{command-stem}.json`
(hand-editable, never rendered into artefacts themselves). A skipped question
SHALL be rendered in the artefact as an explicit `TBD` marker that quotes the
interview question, and the command's user-facing summary SHALL list all
unresolved fields. The interview SHALL surface every derived and authoritative
input (prefilled where available) rather than only the unknown ones; no question
is mandatory, but every question is asked. Interview depth is bounded only by
the set of derived inputs *plus* any authoritative question list the template or
overlay supplies — the interview SHALL NOT invent inputs the template does not
ask for. Upstream artefact dependencies are governed by the existing
prerequisite gating, not by the interview.

#### Scenario: Template's own question block is asked-always
- **WHEN** the effective template carries an `## Intake Interview Questions` section (e.g. the capability-map template's "Which business capabilities exist today…?")
- **THEN** every question in that block is put to the user one at a time (prefilled where available, each skippable) in addition to the inputs derived from the template's substantive sections, and a skipped question renders as a `TBD` marker

#### Scenario: OAA overlay uses its discovery-dimension floor
- **WHEN** an `oaa` overlay command runs its interview
- **THEN** the `intake-discovery-dimensions.md` D1–D10 dimensions are asked-always as the canonical coverage floor (prefilled where resolvable, otherwise a grouped skippable question), adding no diagram or output mandate

#### Scenario: TOGAF ADM command text states the principle
- **WHEN** a TOGAF ADM command's "Run the intake interview" step is read
- **THEN** it states the interview is `ask-always, answer-optional` (the asking is mandatory; each answer is optional/skippable → `TBD`), matching the OAA command wording
