    ...
    for distribution_key in EXPECTED_STANDALONE_REPOS:
        ...
        for pattern in PINNED_README_VERSION_PATTERNS:
            if pattern.search(text):
                failures.append(
                    f"{readme.relative_to(REPO_ROOT)} matches {pattern.pattern}"
                )
...
def test_standalone_version_files_match_root_version():
    ...
    gemini_manifest = json.loads(
        (standalone_path("gemini") / "gemini-extension.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_claude_standalone_marketplace_matches_plugin_version():
    plugin_version = (
        (standalone_path("claude") / "VERSION").read_text(encoding="utf-8").strip()
    )
    ...
    for name, (
        ...
        local_dir,
        license_name,
    ) in EXPECTED_CLAUDE_MARKETPLACE_SOURCES.items():
        plugin = plugins[name]
        manifest = json.loads(
...
def test_root_claude_marketplace_remains_arc_kit_compatibility_marketplace():
    ...
    standalone_plugins = {
        plugin["name"]: plugin for plugin in standalone_marketplace["plugins"]
    }
...
def test_local_claude_standalone_plugin_paths_match_sources():
    ...
    for name, (
        ...
        assert source_path.exists(), (
            f"{name} marketplace source does not exist: {source}"
        )
        ...
        for path in local_path.rglob("*"):
            ...
            assert target not in expected_files, (
                f"duplicate standalone target: {target}"
            )
...
def test_claude_plugin_user_config_entries_have_titles():
    ...
    assert not failures, (
        "Claude plugin userConfig entries missing titles:\n" + "\n".join(failures)
    )
