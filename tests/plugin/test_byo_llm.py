...
from arckit_cli.local import _load_config, _save_config, _ping_endpoint
...
def test_config_save_load(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "arckit_cli.config.DEFAULT_GLOBAL_CONFIG_DIR",
        tmp_path / "config",
    )
...
def test_resolve_config_reads_real_config_file(tmp_path, monkeypatch):
    """P0 regression (unmocked): persisted llm block must flow through resolve_config."""
    from arckit_cli import llm

    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr("arckit_cli.config.DEFAULT_GLOBAL_CONFIG_DIR", tmp_path / "global")
    for var in ("LLM_BASE_URL", "LLM_MODEL", "OPENAI_API_KEY"):
        monkeypatch.delenv(var, raising=False)

    proj = tmp_path / ".arckit" / "config.yaml"
    proj.parent.mkdir(parents=True)
    proj.write_text("llm:\n  base_url: http://file:9\n  model: file-model\n")
    ...
    cfg2 = llm.resolve_config(cli_base_url="http://cli:1", cli_model="cli-model")
    assert cfg2.base_url == "http://cli:1"
    assert cfg2.model == "cli-model"
...
# ---------------------------------------------------------------------------
# Context-window fitting (regression: prompt + max_tokens exceeded context)
# ---------------------------------------------------------------------------

def test_llm_config_defaults_context_window():
    """context_window defaults to the same 128000 as max_tokens."""
    cfg = LLMConfig(
        provider="openai-compatible",
        base_url="http://127.0.0.1:8080",
...
def test_fit_max_tokens_caps_to_context_window():
    """When prompt + max_tokens would exceed the window, max_tokens is capped."""
    cfg = LLMConfig(
        provider="openai-compatible",
        base_url="http://x",
        model="m",
    ...
    # ~8000-char prompt + tool schemas -> estimate well above 0
    messages = [{"role": "user", "content": "x" * 8000}]
    fitted = _fit_max_tokens(messages, [], cfg)
    assert fitted < 128000
    # prompt + fitted must stay under the context window (reserve included)
    prompt_tokens = _estimate_prompt_tokens(messages, [])
    assert prompt_tokens + fitted + 1024 <= 128000
    assert fitted >= 256


def test_fit_max_tokens_unchanged_when_fits():
    """A small max_tokens is passed through untouched when it already fits."""
    cfg = LLMConfig(
        provider="openai-compatible",
        base_url="http://x",
        model="m",
    ...
    assert _fit_max_tokens(messages, [], cfg) == 1024


def test_fit_max_tokens_floor():
    """Huge prompt never drives the completion budget below the floor."""
    cfg = LLMConfig(
        provider="openai-compatible",
        base_url="http://x",
        model="m",
    ...
    assert _fit_max_tokens(messages, [], cfg) == 256


@pytest.mark.asyncio
async def test_call_llm_sends_fitted_max_tokens():
    """The wire request uses the fitted completion budget, not the raw default."""
    cfg = LLMConfig(
        provider="openai-compatible",
        base_url="http://localhost:8080",
        model="test",
    ...
    messages = [{"role": "user", "content": "x" * 8000}]
    with patch("arckit_cli.llm.httpx.AsyncClient", return_value=_mock_client(post)):
        await call_llm(messages=messages, tools=[], config=cfg)

    sent = captured["body"]["max_tokens"]
    assert sent < 128000
    prompt_tokens = _estimate_prompt_tokens(messages, [])
    assert prompt_tokens + sent + 1024 <= 128000
